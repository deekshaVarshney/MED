...
if __name__ == '__main__':
    ...
    parser.add_argument('--load_dir', default='weights/english/kg/covid_mlm/', type=str, required=False)
    parser.add_argument('--validate_load_dir', default='../../preprocessed_data/kg_data_pointer/data_biobert/c_data_mlm'
                                                       '/validate_data.pkl', type=str, required=False)
    parser.add_argument('--train_load_dir', default='../../preprocessed_data/kg_data_pointer/data_biobert/c_data_mlm/train_data.pkl', type=str, required=False)
    parser.add_argument('--log_dir', default='log/train_mlm.txt', type=str, required=False)
    parser.add_argument('--val_epoch_interval', default=1, type=int, required=False)
    parser.add_argument('--last_epoch_path', default="weights/english/kg/covid_mlm/", type=str, required=False)
    ...
    model = transformers_model(args.model_config, vocab_size)
    ...
    if finetune_check == 'true' or finetune_check == 'True':
        ...
        checkpoint = torch.load(PATH + 'bestmodel.pth', map_location='cuda:6')
        print(PATH + 'bestmodel.pth')
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        loss = checkpoint['loss']
    ...
    for epoch in range(epochs):
        ...
        for batch in train_dataloader:
            ...
            encoder_input, decoder_input, mask_encoder_input, mask_decoder_input, labels = batch
            logits = model(encoder_input, mask_encoder_input, decoder_input, mask_decoder_input, labels)
