...
class transformers_model(nn.Module):
    def __init__(self, config, vocab_size):
        ...
        encoder_config_bio = AutoConfig.from_pretrained("./weights/biobert_weight/dmis_biobert_large_case/config.json")
        # encoder_config = BertConfig.from_json_file(self.config)
        self.mlm = BertForMaskedLM(encoder_config)

        self.encoder = AutoModel.from_pretrained('./weights/biobert_weight/dmis_biobert_large_case/', config=encoder_config_bio)
        ...
        self.linear_mlm = nn.Linear(512, 1024, bias=False)
        #self.linear = nn.Linear(768, 28996, bias=False)
        #---- Linear Layer for BioBert large case ------
        self.linear = nn.Linear(1024, vocab_size, bias=False)

    def forward(self, input_ids, mask_encoder_input, output_ids, mask_decoder_input, labels):
        # encoder_hidden_states: [batch_size, max_length, hidden_size]
        # encoder_hidden_states, _ = self.encoder(input_ids, mask_encoder_input)
        # print(encoder_hidden_states.size())
        encoder_hidden_states = self.mlm(input_ids, attention_mask= mask_encoder_input, labels=labels, output_hidden_states=True)
        encoder_hidden_states = self.linear_mlm(encoder_hidden_states.hidden_states[-1])
        # out: [batch_size, max_length, hidden_size]
        # print(encoder_hidden_states[-1].size())
        out = self.decoder(output_ids, mask_decoder_input, encoder_hidden_states=encoder_hidden_states)
        out = out.last_hidden_state
